...
assert.equal(R.canLearnSpell(null, 'wizard', 3), false)
assert.equal(R.canLearnSpell(real.spells.fireball, 'fighter', 9), false)
assert.equal(R.canLearnSpell(real.spells.fireball, 'wizard', 0), false)
assert.equal(R.maxSlotLevel('none', 10), 0)
assert.equal(R.maxSlotLevel('half', 2), 1)
assert.equal(R.maxSlotLevel('warlock', 5), 3)
assert.equal(R.maxSlotLevel('full', 1), 1)
assert.equal(R.maxSlotLevel('full', 20), 9)
let pick = R.togglePick([], 'a', 2)
assert.deepEqual(pick.arr, ['a'])
pick = R.togglePick(pick.arr, 'b', 2)
assert.deepEqual(pick.arr, ['a', 'b'])
const fullPick = R.togglePick(pick.arr, 'c', 2)
assert.equal(fullPick.ok, false)
assert.deepEqual(fullPick.arr, ['a', 'b'])
const off = R.togglePick(pick.arr, 'a', 2)
assert.equal(off.ok, true)
assert.deepEqual(off.arr, ['b'])
assert.equal(R.togglePick(['a'], '', 2).ok, false)
assert.deepEqual(R.togglePick(undefined, 'a', 1).arr, ['a'])
...
const monk1 = R.createCharacter({ name: '僧', race: 'human', class: 'monk', level: 1, abilities: { str: 10, dex: 16, con: 14, int: 8, wis: 15, cha: 8 } }, real)
assert.ok(!(monk1.resources || []).some(r => r.id === 'ki'))
const monk2 = R.createCharacter({ name: '僧', race: 'human', class: 'monk', level: 2, abilities: { str: 10, dex: 16, con: 14, int: 8, wis: 15, cha: 8 }, hpMax: 16 }, real)
assert.ok(monk2.resources.some(r => r.id === 'ki' && r.max === 2))
const cleric2 = R.createCharacter({ name: '牧', race: 'human', class: 'cleric', level: 2, abilities: { str: 10, dex: 10, con: 14, int: 8, wis: 16, cha: 12 }, hpMax: 16 }, real)
assert.ok(cleric2.resources.some(r => r.id === 'channel-divinity' && r.max === 1))
const bm3 = R.setSubclass(R.createCharacter({ name: '師', race: 'human', class: 'fighter', level: 3, abilities: { str: 16, dex: 12, con: 14, int: 10, wis: 10, cha: 8 }, hpMax: 28 }, real), 'battlemaster', real)
assert.ok(bm3.resources.some(r => r.id === 'superiority' && r.max === 4))
assert.equal(R.canWearArmor({ class: 'wizard', subclass: null }, 'plate', real), false)
assert.equal(R.canWearArmor({ class: 'fighter', subclass: null }, 'plate', real), true)
const wizNo = R.setArmor(R.createCharacter({ name: '法', race: 'human', class: 'wizard', level: 1, abilities: { str: 8, dex: 14, con: 13, int: 16, wis: 10, cha: 8 } }, real), 'plate', real)
assert.equal(wizNo.armor, null)
assert.equal(R.canLearnSpell(real.spells['charm-person'], 'wizard', 1, { subclass: 'eldritch-knight' }), false)
assert.equal(R.canLearnSpell(real.spells['charm-person'], 'wizard', 1, { subclass: 'eldritch-knight', allowOffSchool: true }), true)
